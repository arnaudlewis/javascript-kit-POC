...
const DefaultFragments = require('./default');
/**
...
function DocumentLink(data) {
  ...
  const doc = {
     ...
     */
    '_contentType': data.document.type,
    '_data': data,
    /**
     * @field
  ...
  return R.merge(doc, parseFragments(fragmentsData));
}
    ...
    if (tagsStart[pos]) {
      ...
      tagsStart[pos].forEach(function (span) {
        ...
        if (span.type == "hyperlink") {
          ...
          if (fragment) {
            url = fragment._type === 'DocumentLink' ? fragment.url(linkResolver) : fragment.url;
          } else {
...
function SliceZone(mask, data) {
  ...
  const slices = checkedData.map((sliceData) => {
    var partialMask = mask;
    for (var key in mask) {
      if (sliceData.slice_type == key) {
        partialMask = { group: mask[key].config.fields}
      }
    }
    return Slice(partialMask, sliceData);
  });
...
/**
 * Parse the fragment from the document mask, casting either the found value or a default value
 *
 * @private
 * @param {string} field - the fragment's name
 * @returns {object} - the object of the proper Fragments type or default value.
 */
function parseFragments(json, mask) {
  let result = {};
  let withNoMatch = true;

  for (var tabKey in mask) {
    var tabMask = mask[tabKey];
    for (var maskFieldKey in tabMask) {
      if (maskFieldKey != "uid"){

        for (var jsonFieldKey in json) {
          result = R.merge(parseBasic(mask, tabMask, maskFieldKey, json, jsonFieldKey), result);
        }
...
/**
 * Parse a basic fragment with either the found value or a default value
 *
 * @private
 * @param {string} field - the fragment's name
 * @returns {object} - the object of the proper Fragments type or default value.
 */
function parseBasic(fullMask, subMask, maskKey, json, key) {
  const keyID = key.substring(key.indexOf(".") + 1)
  const resKey = keyID === maskKey ? key : maskKey;

  const partialMask = getSubMask(fullMask, subMask[maskKey])
  const resValue = json.hasOwnProperty(key)
    ? initField(fullMask, json[key])
    : DefaultFragments.defaultByField(subMask[maskKey]);

  return R.assoc(resKey, resValue, {});
}

function getSubMask(mask, fieldValue) {
  switch(fieldValue.type) {
    case 'Group' : return fieldValue.config.fields;
    case 'Slices' : return fieldValue.config.choices;
    default: return mask;
  }
}
