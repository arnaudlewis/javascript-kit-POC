...
function parseFragments(json, mask) {
  ...
  var result = {};
  for (var tab in mask) {
    var tabFields = mask[tab];
    for (var maskKey in tabFields) {
      if (maskKey != "uid"){
        var withNoMatch = true;
        for (var key in json) {
          var keyID = key.substring(key.indexOf(".") + 1);
          if (keyID == maskKey) {
            withNoMatch = false;
            let partialMask = mask; 
            if ( tabFields[maskKey].type == "Group" ) {
              partialMask = { group: tabFields[maskKey].config.fields};
            } else if ( tabFields[maskKey].type == "Slices" ) {
              partialMask = tabFields[maskKey].config.choices;
            }
            result[key] = parseBasic(partialMask, json, key);
          }
        }
        if (withNoMatch) {
          var defaultValue = require('./default').defaultByField(tabFields[maskKey]);
          result[maskKey] = defaultValue;
        }
      }
    }
  }
 ...
 * @returns {object} - the object of the proper Fragments type or default value.
 */
function parseBasic(mask, json, key) {
  var initValue;
  if (json.hasOwnProperty(key)) {
    initValue = initField(mask, json[key]);
  }
  return initValue;
}
