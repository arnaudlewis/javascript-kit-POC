...
function defaultByField(field) {
  ...
  if (field.type == "Link") {
    if (field.config.select) {
      if (field.config.select == "document") return DocumentLink();
      if (field.config.select == "media") return FileLink();
      if (field.config.select == "web") return WebLink();
    } else {
      return WebLink();
    }
  } else if (defaultForType[field.type]) {
